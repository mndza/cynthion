...
from amaranth                       import Elaboratable, Module, Signal, Cat, Memory, ResetSignal, Mux
...
class PaddingRemover(Elaboratable):
    ...
    def elaborate(self, platform):
        ...
        length      = Signal(16)
        alignment   = Signal(range(self.alignment))
        skip        = Signal()
        ...
        with m.If(self.output.ready | ~self.output.valid):
            ...
            m.d.sync += [
                ...
                self.output.valid   .eq(self.input.valid & ~skip),
            ]
            ...
            with m.If(self.input.valid):
                ...
                with m.FSM():
                    ...
                    with m.State("LENGTH_0"):
                        m.d.sync += alignment.eq(self.alignment - 1)
                    ...
                    with m.State("PAYLOAD"):
                        ...
                        m.d.sync += alignment.eq(Mux(alignment == 0, self.alignment - 1, alignment - 1))
                        with m.If(length == 1):
                            m.d.comb += self.output.last.eq(1)
                            with m.If(alignment == 0):
                                m.next = "LENGTH_0"
                            with m.Else():
                                m.next = "PADDING"
                    ...
                    with m.State("PADDING"):
                        m.d.comb += skip.eq(1)
                        m.d.sync += alignment.eq(alignment - 1)
                        with m.If(alignment == 0):
                            m.next = "LENGTH_0"
